...
class RolloutEvaluator:
    ...
    def on_obstacle_margin(self, margin):
        if self.finished:
            return
    ...
    def result(self):
        ...
        if self.start_time is not None:
            ...
            if end_time is not None:
                flight_time = max(0.0, float(end_time) - self.start_time)
        ...
        return {
            ...
            "termination_elapsed_time": flight_time,
...
def evaluate_rows(rows, profile):
    ...
    for row in rows:
        ...
        if "nearest_obstacle_margin" in row and row["nearest_obstacle_margin"] not in ("", None):
            evaluator.on_obstacle_margin(float(row["nearest_obstacle_margin"]))
        elif "is_collide" in row:
            evaluator.on_obstacle_margin(float(row["is_collide"]) > 0)
