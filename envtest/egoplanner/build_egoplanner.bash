...
WORKSPACE="${VITFLY_EGOPLANNER_WORKSPACE:-$REPO_ROOT/../../../.planner_workspaces/egoplanner}"
SOURCE="$WORKSPACE/src/ego-planner"
VENDOR_SOURCE="$REPO_ROOT/third_party/ego-planner"
EGO_COMMIT="bfda51284c8c1b476043255a8145ef925a3778a5"

source /opt/ros/noetic/setup.bash
mkdir -p "$WORKSPACE/src"

if [ "$(git -C "$VENDOR_SOURCE" rev-parse HEAD 2>/dev/null || true)" != "$EGO_COMMIT" ]; then
  echo "[EGOPLANNER BUILD] initialize third_party/ego-planner at $EGO_COMMIT first" >&2
  exit 2
fi
if [ -e "$SOURCE" ] && [ ! -d "$SOURCE/.git" ]; then
  echo "[EGOPLANNER BUILD] generated source is not a git worktree: $SOURCE" >&2
  exit 2
fi
if [ ! -d "$SOURCE/.git" ]; then
  git clone --local "$VENDOR_SOURCE" "$SOURCE"
fi
git -C "$SOURCE" reset --hard "$EGO_COMMIT" >/dev/null
