...
void UnityObject::run(const Scalar dt) {
  ...
  const Scalar traj_dt = std::max(traj_[1].t - traj_[0].t, dt);
  int idx = int(state_.t / traj_dt);

  if (loop_ && state_.t >= traj_.back().t) {
    state_.t = 0.0;
    idx = 0;
    sign_ = 1.0;
  } else if (idx <= 0) {
...
bool UnityObject::loadTrajectory(const std::string traj_csv) {
  ...
  for (auto& row : CSVRange(infile)) {
    ...
    //
    state_i.x[RS::ATTW] = std::stod((std::string)row[4]);
    state_i.x[RS::ATTX] = std::stod((std::string)row[5]);
    state_i.x[RS::ATTY] = std::stod((std::string)row[6]);
    state_i.x[RS::ATTZ] = std::stod((std::string)row[7]);
